...
public:
	...
	Molecola isomerizeALLR();	// try to isomerize the alk ro, if it is possible return the isomer, otherwise return the same molecule
	...
	bool isCradVinylic(int pos); // return true if the radical in position pos is vynilic

	std::string inchiName();

	int generatedByLowTemperature = -1;  // track if the species was generated by a low-temperature subemchanism's reaction
										 // -1 = non specifiec; 0 = no; 1 = yes
};
